<template>
	<view class="bg-white">
		<view class="fixed">
			<cu-custom bgColor="bg-shadeTop text-white" :isBack="true"><block slot="backText">返回</block><block slot="content">我的月啊</block></cu-custom>
		</view>
		<image src="/static/bannerBG.png"
		 ...
		 <view class="cu-bar bg-white solid-bottom">
		 	<view class="action">
		 		<button class="cu-btn round shadow margin-right" :class="limitFlag==-1?'lines-blue':'lines-gray'" @click="changeLimit(-1)">上月</button>
		 		<button class="cu-btn round shadow" :class="limitFlag==0?'lines-blue':'lines-gray'" @click="changeLimit(0)">本月</button>
		 	</view>
		 	<view class="action">
		 	</view>
		 </view>
...
<script>
	...
	import {getReportMonths} from '../../common/api.js'
	var _self=null;
	var canvaPie=null;
	...
	export default {
		...
		methods: {
			fetchData(){
				getReportMonths(this.limitFlag).then((res) => {
					...
					ColumnStack.categories=res.data3categories;
			...
			showColumn(canvasId,chartData){
				canvasColumn=new uCharts({
					...
					series: chartData.series,
					enableScroll: (chartData.categories.length> 8?true:false),
					xAxis: {
						disableGrid:true,
						itemCount:(chartData.categories.length> 8 ? 8: chartData.categories.length),
						scrollShow:true,
						scrollAlign:'left',
					},
					yAxis: {
